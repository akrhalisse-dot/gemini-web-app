...
import { ArrowRight, ArrowDown, Plus, MapPin, Mail, Phone, Instagram, Linkedin, Menu, X } from 'lucide-react';
...
interface ProjectData {
  id: string;
  title: string;
  concept: LocalizedString; // Nouveau champ pour le sous-titre conceptuel du PDF
  year: string;
  img: string;
...
// --- 2. DATA CONSTANTS (TEXTES BASÉS SUR LE PDF) ---
...
const PROJECTS: ProjectData[] = [
  {
    ...
    desc: { 
      fr: "Réponse sculpturale à la densité urbaine, ce projet refuse la monotonie du bloc traditionnel. Chaque appartement est pensé comme une villa suspendue, prolongée par de profonds espaces extérieurs.", 
      en: "A sculptural response to urban density refusing traditional monotony. Each apartment is designed as a suspended villa with deep outdoor spaces.", 
      ar: "استجابة نحتية للكثافة الحضرية ترفض رتابة الكتل التقليدية. تم تصميم كل شقة كفيلا معلقة مع مساحات خارجية عميقة." 
    }
  },
  {
    id: "02",
    ...
    desc: { 
      fr: "Une réinterprétation contemporaine de l'architecture sacrée. Conçu comme un monolithe de lumière, l'édifice s'ancre dans la ville avec une puissance silencieuse, filtrant le soleil via une résille paramétrique.", 
  ...
  },
  {
    ...
    desc: { 
      fr: "Architecture du néo-vernaculaire où le bâti semble émerger de la terre. La pierre sèche locale et le béton brut dialoguent avec la minéralité du site pour créer un refuge intemporel.", 
  ...
  },
  {
    ...
    category: { fr: "RÉSIDENTIEL", en: "Residential", ar: "سكني" },
    desc: { 
      fr: "Hybridation des codes classiques avec une écriture épurée. Nous avons revisité la silhouette archétypale de la maison à toiture tuiles en purifiant ses lignes et en cadrant le paysage.", 
      en: "Hybridizing classic codes with a clean style. We revisited the archetypal silhouette of the tiled-roof house by purifying its lines and framing the landscape.", 
...
const TRANSLATIONS = {
  fr: {
    nav: { projects: "Projets", expertise: "Expertise", agency: "L'Agence", contact: "Contact", open_menu: "Menu", close_menu: "Fermer" },
    hero: { 
    ...
    portfolio: { label: "Sélection de Projets", view_project: "DÉCOUVRIR", view_all: "Voir tout le portfolio" },
    expertise: { 
  ...
  en: {
    nav: { projects: "Projects", expertise: "Expertise", agency: "Agency", contact: "Contact", open_menu: "Menu", close_menu: "Close" },
    ...
    portfolio: { label: "Selected Projects", view_project: "DISCOVER", view_all: "View full portfolio" },
  ...
  ar: {
    nav: { projects: "المشاريع", expertise: "الخبرة", agency: "الوكالة", contact: "اتصل بنا", open_menu: "قائمة", close_menu: "إغلاق" },
    ...
    portfolio: { label: "مختارات من المشاريع", view_project: "اكتشف", view_all: "عرض المحفظة الكاملة" },
    ...
    <>
      <header className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 border-b ${scrolled ? 'bg-black/90 backdrop-blur-md border-neutral-900 h-20' : 'bg-transparent border-transparent h-24'} flex justify-between items-center px-6 md:px-12`}>
        ...
        <nav role="navigation" className="hidden md:flex items-center gap-8 font-mono text-xs uppercase tracking-widest text-neutral-400">
          {['portfolio', 'expertise', 'agency', 'contact'].map(item => (
                ...
                <div className="border-t border-neutral-800 pt-6 mb-8 flex justify-between items-start">
                  <div className="flex gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-sans text-sm text-white mb-2 italic opacity-80">{data.concept[lang]}</p>
    ...
    <section id="expertise" className="relative z-10 px-6 md:px-12 py-32 bg-neutral-950 border-b border-neutral-900" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      <div className="max-w-[1920px] mx-auto flex flex-col lg:flex-row lg:justify-between lg:items-end gap-16 lg:gap-32">
        <div className="flex-1 lg:max-w-xl">
          ...
          <Reveal delay={300}>
            {/* Using the Villa Extension Bois image here or another detail image if preferred */}
